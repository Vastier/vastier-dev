...
const buttonVariants = cva('active:scale-95 inline-flex items-center justify-center rounded-md text-sm font-medium transition-color focus:outline-none focus:ring-slate-400 focus:ring-offset-2 disabled:opacity-50 dark:focus:ring-slate-400 dark:focus:ring-offset-slate-900 disabled:pointer-events-none', {
	variants: {
		variant: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({
	...
	return (
		<button
			...
			disabled={isLoading}
			{...props}>
			{isLoading ? <Loader2 className='mr-2 h-4 w-4 animate-spin' /> : <div className='hidden' />}
			{children}
		</button>
	)
  }
